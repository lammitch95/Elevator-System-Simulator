...
public class MedicalElevatorTwo extends Elevators{
    ...
    public void upRequests(ArrayList<ArrayList<Passengers>> floors,int[] cf,ArrayList<Rectangle> genElev,ArrayList<Rectangle> bases,ArrayList<Rectangle> genOne_DOORS, Label capacityGUI, Label currentFloorGUI,Rectangle lWall, Rectangle rWall,Label errorDisplayGUI){
        ...
        for(int i = cf[6]; i < getMax_Floors(); i++){//this for loop checks between current floor and highest floor to get Elevator request
            for(int j = 0; j < floors.get(i).size(); j++){
                Passengers p = floors.get(i).get(j);
                if(p.getElv_Num() == 7 && (p.getPassType().equals("Support")||p.getPassType().equals("Medical")||p.getPassType().equals("Security")) && (p.getEndFloor() > p.getStartFloor())&& inGeneralElevator.size()<= getMax_Capacity() ){
                    nextFloorRequest= i;
                    requestCheck = true;
                    break; 
                }
        ...
        if(!inGeneralElevator.isEmpty()){
            ...
            for(int j = 0; j < inGeneralElevator.size(); j++){
                ...
                if(inGeneralElevator.get(j).getEndFloor() < min && inGeneralElevator.get(j).getEndFloor() > cf[6]){
       ...
       if(requestCheck == false){
        ...
        tr.setNode(genElev.get(6));
        tr.setDuration(Duration.seconds(1));
        tr.setCycleCount(1);
        tr.setByY((nextFloorRequest - cf[6]) *-65);
        ...
        tr.setOnFinished(finish ->{
            cf[6] = nextFloorRequest;
            currentFloorGUI.setText(Integer.toString(cf[6]));
            System.out.println("General Elevator 1 Currently on Floor: " + cf[6]);
            ...
            System.out.println("General Elevator 1 Doors Opening on Floor: " + cf[6]);
            ...
            for(int i = 0; i < inGeneralElevator.size(); i++){
                if(inGeneralElevator.get(i).getEndFloor() == cf[6]){
                    getDestination_Floors().get(cf[6]).add(inGeneralElevator.get(i));

                    System.out.println("Passenger: " + inGeneralElevator.get(i).toString() + " has exited Medical Elevator 1 on Floor " + cf[6]);
                    elevatorCap--;
                    capacityGUI.setText(Integer.toString(elevatorCap)+"/4");
                    inGeneralElevator.get(i).getPassGUI().setLayoutX(genElev.get(6).getX()+30);
                    inGeneralElevator.get(i).getPassGUI().setLayoutY(genElev.get(6).getY());
            ...
            inGeneralElevator.removeAll(getDestination_Floors().get(cf[6]));
        
            for(int i = 0; i < floors.get(cf[6]).size(); i++){
                if(inGeneralElevator.size()>=getMax_Capacity()){
                    capacityGUI.setTextFill(Color.web("#ff0000"));
                    break;
                }
                Passengers p = floors.get(cf[6]).get(i);
                if((p.getPassType().equals("Support")||p.getPassType().equals("Medical")||p.getPassType().equals("Security")) && p.getElv_Num() == 7   && p.getEndFloor() > p.getStartFloor() && inGeneralElevator.size()<= getMax_Capacity()){
            ...
            floors.get(cf[6]).removeAll(inGeneralElevator);
        ...
        errorDisplayGUI.toFront();
         ...
         System.out.println("General Elevator 1 Doors Closing on Floor: " + cf[6]);
    ...
    public void elevatorRequestCheck(ArrayList<ArrayList<Passengers>> floors,int[] cf,ArrayList<Rectangle> genElev,ArrayList<Rectangle> bases,ArrayList<Rectangle> genOne_DOORS,Label capacityGUI, Label currentFloorGUI,Rectangle lWall, Rectangle rWall,Label errorDisplayGUI){
        ...
        for(int i = cf[6]; i < getMax_Floors();i++){
            for(int j = 0; j < floors.get(i).size(); j++){
                Passengers p = floors.get(i).get(j);
                 if(p.getElv_Num() == 7  && (p.getPassType().equals("Support")||p.getPassType().equals("Medical")||p.getPassType().equals("Security")) && (p.getEndFloor() < p.getStartFloor()) && inGeneralElevator.size() <= getMax_Capacity()){
                    nextFloorRequest = i;
            ...
            if(nextFloorRequest > cf[6]){
                requestCheck = true;
            }
    
        } 
        
        if(nextFloorRequest == cf[6] && requestCheck == false){
            ...
            tr.setByY((cf[6]-nextFloorRequest)*65);
            ...
            tr.setOnFinished(finish4 ->{
                ...
                nextFloorRequest = 0;
                requestCheck = false;
                for(int i = cf[6]; i < 8; i++){
                    
                    for(int j = 0; j < floors.get(i).size(); j++){
                        Passengers p = floors.get(i).get(j);
                        if(p.getElv_Num() == 7  && (p.getPassType().equals("Support")||p.getPassType().equals("Medical")||p.getPassType().equals("Security")) && (p.getEndFloor() > p.getStartFloor()) ){
                            requestCheck = true;
                            upRequests(floors,cf,genElev,bases,genOne_DOORS,capacityGUI,currentFloorGUI,lWall,rWall,errorDisplayGUI);
                            break; 
                        }
                ...
                if(requestCheck == false){
                    ...
                    setMedicalElevatorTwoCall(true);
                    cf[6]= -1;
                }
            ...
            System.out.println("General Elevator 3 Doors Opening on Floor: " + cf[6]);
            p.setOnFinished(finish2 ->{
       ...
       errorDisplayGUI.toFront();
         ...
         System.out.println("General Elevator 3 Doors Closing on Floor: " + cf[6]);
                   ...
                   for(int i = cf[6]; i < getMax_Floors(); i++){//this for loop checks between current floor and highest floor to get Elevator request
                        for(int j = 0; j < floors.get(i).size(); j++){
                            ...
                            if(p.getElv_Num() == 7  && (p.getPassType().equals("Support")||p.getPassType().equals("Medical")||p.getPassType().equals("Security")) && (p.getEndFloor() > p.getStartFloor())  ){
                                ...
                                upRequests(floors,cf,genElev,bases,genOne_DOORS,capacityGUI,currentFloorGUI,lWall,rWall,errorDisplayGUI);
                                break; 
                            }else if(p.getElv_Num()==7 && (p.getPassType().equals("Support")||p.getPassType().equals("Medical")||p.getPassType().equals("Security")) && (p.getEndFloor() < p.getStartFloor())  ){
                                nextFloorRequest= i;
                                requestCheck = true;
                                elevatorRequestCheck(floors,cf,genElev,bases,genOne_DOORS,capacityGUI,currentFloorGUI,lWall,rWall,errorDisplayGUI);
                                break;
